...
sub_age = '23yrs';
sub_result = strcat('C:\Users\byron\Desktop\HippoCode20240603\results\sub_', sub_num, '_', sub_name, '_v7_run3.mat');
...
try
    ...
    % 实验帧数的设计
    n_trial=15; % 3 for exercise; 15 for run
    
    trial_interval=18; trial_interval=time2frame(flipIntv,trial_interval); % 试次之间的间隔帧数，可以设置为被试作出反应的时间
    fix_onset=1; fix_onset=time2frame(flipIntv,fix_onset); % 注视点的帧数
    image_onset=3; image_onset=time2frame(flipIntv,image_onset); % 每张图片呈现的帧数
    hold_onset=10; hold_onset=time2frame(flipIntv,hold_onset); % 等待记忆保留的帧数
    response_onset=4; response_onset=time2frame(flipIntv,response_onset); % 作出反应的最长时间
    ...
    for i=1:n_trial
        ...
        % ===刺激间隙================================================
        if(K_down)
            response_int_onset=time2frame(flipIntv,4-RT_pool(i));
            Screen('FillRect',win,0);
            time_stamp = Screen('Flip',win,time_stamp+(response_int_onset-0.5)*flipIntv);
        else
            response_int_onset=time2frame(flipIntv,4);
            Screen('FillRect',win,0);
            time_stamp = Screen('Flip',win,time_stamp+(response_int_onset-0.5)*flipIntv);
        end
